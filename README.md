...
![いつもの改行 for Excel の画面](assets/screenshot.png)
...
Excelのセル編集中判定は、Excelのバージョンや更新状況によって変わる場合があります。
動作しない環境があれば、Excelのバージョン、Windowsのバージョン、選択している改行キーを添えてお知らせください。
...
- Excelが前面にあるか確認してください。
- セルを選択しているだけではなく、セル内で文字入力中か確認してください。
- `Enter` モードでは、セル内入力中と判断できない場合は変換しません。
- `Shift + Enter` モードでは、セルを選択しているだけの状態でも、セル側だと判断できれば変換します。
- 図形・テキストボックスを編集中の場合は変換しません。
- IME変換中のEnterは、改行ではなく変換確定を優先します。
- Excelのバージョンや更新状況によって、セル編集中の判定が合わない場合があります。
- うまく動かない場合は、アプリを再起動してから再度確認してください。
